...
function DecodeCroatianMojibake($text) {
    # It appears PowerShell gets these strings as ISO-8859-2, CP1250, or similar mixed up with UTF-8.
    # The output shows  which is \uFFFD. This happens during Get-Content / Write-Host, but inside the docx it's pure UTF-8 xml.
    # If we map straight from memory, we can preserve it if we don't console out.
    return $text
}
...
# Write directly using bytes to guarantee UTF8 without BOM
[System.IO.File]::WriteAllBytes("$pwd\glagoljasko_pjevanje_lokacije.json", [System.Text.Encoding]::UTF8.GetBytes($json))
...
[System.IO.File]::WriteAllBytes("$pwd\glagoljasko_pjevanje_lokacije.csv", [System.Text.Encoding]::UTF8.GetBytes(($csv -join "`r`n")))
