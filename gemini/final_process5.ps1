function Fix-DocxExtraction ($path) {
    ...
    $stream = $entry.Open()
    # Read bytes instead of depending on StreamReader encoding heuristics
...
foreach ($row in $global:allData) {
    ...
    if ($loc -eq '' -or $loc -eq 'Nepoznato' -or $loc -match 'NAPJEV' -or $loc -match '^\(MJESTO SNIMANJA NIJE NAVEDENO$') { continue }
